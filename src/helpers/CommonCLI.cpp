...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {  // Legacy prefs loader
  ...
  if (file) {
    ...
    // next: 295 -- legacy binary format predates RX powersaving; rx_ps_* fields
    // are not present in it and stay at their zero defaults (see ensureRxPowerSavingDefaults())

...
void NEH7100::enableTempLimit(bool on)
{
  ...
  _temp_enabled = on;
  _temp_active = false;
  if (on) {          // fresh start: forget any earlier forced-charge episode
    _batt_forced = false;
    _low_strikes = 0;
  }
}

// Rewrite the OVP field, leaving the LVD half of the register alone.
void NEH7100::writeOvpCode(uint8_t code)
{
  uint8_t r;
  if (readReg(REG_OVP_LVD, &r)) writeReg(REG_OVP_LVD, (r & 0xF0U) | (code & 0x0FU));
}
...
void NEH7100::tick(float mcuTempC, uint16_t battMv)
{
  ...
  // A transmit burst sags the rail, so one low reading means nothing. Only a
  // run of them reports real discharge.
  if (battMv > 0 && battMv < critical) {
    if (_low_strikes < 255) _low_strikes++;
  } else {
    _low_strikes = 0;
  }

  if (_batt_forced) {
    // charging is forced until the pack recovers, then the limit is re-armed
    if (battMv > 0 && battMv > critical + BATT_RECOVER_MV) {
      _batt_forced = false;
      _low_strikes = 0;
      Serial.println("[NEH7100] battery recovered, temp limit re-armed");
    }
    return;
  }

  if (_low_strikes >= BATT_LOW_STRIKES) {
    // Battery before temperature: let charging run, but keep the guard so it
    // returns once the pack has recovered instead of staying off until reboot.
    _batt_forced = true;
    _low_strikes = 0;
    if (_temp_active) {
      writeOvpCode(_ovp_code);
      _temp_active = false;
    }
    Serial.println("[NEH7100] battery critical: charging forced, temp limit suspended");
    return;
  }
  ...
  if (_temp_active) {
    ...
    writeOvpCode(0x00U);   // OVP 2.7 V stops charging
    _temp_active = true;
...
bool NEH7100::handleCommand(char* cmd, char* reply, float mcuTempC)
{
  ...
  // Raw current-measurement registers. The gain range is whatever the PMIC has
  // selected itself - nothing in this driver writes it - so this is the only way
  // to see whether a reading is clipped at the top of its range.
  if (strcmp(cmd, "neh.raw") == 0) {
    uint8_t range, meas;
    if (!readReg(REG_I_RANGE, &range) || !readReg(REG_I_MEAS, &meas)) {
      strcpy(reply, "ERR: NEH7100 I2C read failed");
      return true;
    }
    sprintf(reply, "> Ireg=0x%02X,range=%u,Imeas=%u,Ibat=%ldnA",
            (unsigned)range, (unsigned)(range & 0x03U), (unsigned)meas,
            (long)chargeCurrentNa());
    return true;
  }
  ...
  if (strcmp(cmd, "neh.temp") == 0) {
    sprintf(reply, "> %s min=%dC max=%dC%s",
            ...
            _batt_forced ? " FORCED" : (_temp_active ? " LIMITED" : ""));
    return true;
  }
  ...
  if (strcmp(cmd, "neh.temp off") == 0) {
    if (_temp_active) writeOvpCode(_ovp_code);
    _temp_active = false;
    _temp_enabled = false;
    _batt_forced = false;
    _low_strikes = 0;

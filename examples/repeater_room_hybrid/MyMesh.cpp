...
void MyMesh::onAdvertRecv(mesh::Packet *packet, const mesh::Identity &id, uint32_t timestamp,
  ...
  // if this is a zero hop advert (and not via 'Share'), add it to neighbours
...
int MyMesh::handleRequest(ClientInfo *sender, uint32_t sender_timestamp, uint8_t *payload,
  ...
  if (payload[0] == REQ_TYPE_GET_NEIGHBOURS) {
    int reply_offset = 4; // after response sender_timestamp/tag
      ...
      telemetry(MAX_PACKET_PAYLOAD - 4)
{
  ...
  _prefs.disable_fwd = 0;   // hybrid: forward by default (repeater role)

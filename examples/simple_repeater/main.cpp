...
void loop() {
  ...
  // loop guard `len < sizeof(command)-1` into `len != ...`, and deletes any
  // clamp - so a huge len slips through and command[len++] writes far out of
  // bounds (this is the wild writer that corrupted _next_idx in the crash
  // dumps). Scanning manually up to the buffer size is well-defined.
  int len = 0;
